...
export async function searchProducts(query, filters, maxPages, emitLog, shouldStop, requiredTitleKeywords = []) {
  const page = await newPage();
  const allProducts = [];
  const titleKeywords = normalizeTitleKeywords(requiredTitleKeywords);

  try {
    const url = new URL(BASE_SEARCH_URL);
    url.searchParams.set('q', query);
    emitLog(`🔍 AND 搜索: "${query}" (最多${maxPages}页)`);
    ...
    await applySort(page, filters.searchSort, emitLog);
    await sleep(2000);
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      const titleMatchedProducts = products.filter(product => titleMatchesAllKeywords(product.title, titleKeywords));
      emitLog(`  找到 ${products.length} 个商品，标题 AND 匹配 ${titleMatchedProducts.length} 个`);
      if (titleMatchedProducts.length === 0 && products.length > 0 && titleKeywords.length > 0) {
        const samples = products.slice(0, 3).map(product => {
          const missing = titleKeywords.filter(keyword => !normalizeText(product.title).includes(normalizeText(keyword)));
          return `“${String(product.title || '').slice(0, 45)}” 缺少: ${missing.join('、') || '未知'}`;
        });
        emitLog(`  标题匹配诊断: ${samples.join(' | ')}`);
      }
      allProducts.push(...titleMatchedProducts);
  ...
  emitLog(`🔍 "${query}" 搜索完成，共采集 ${allProducts.length} 个标题 AND 匹配商品`);
  return allProducts;
}
...
function titleMatchesAllKeywords(title, keywords) {
  const normalizedTitle = normalizeText(title);
  return keywords.every(keyword => normalizedTitle.includes(normalizeText(keyword)));
}
...
async function applyFilters(page, filters, emitLog) {
  try {
    ...
    for (const [key, label] of [
      ['requireInspect', '验货宝'], ['requireGuarantee', '验号担保'], ['requireFreeShipping', '包邮'],
      ['requireSuperShop', '超赞鱼小铺'], ['requireBrandNew', '全新'], ['requireStrictSelect', '严选'], ['requireResale', '转卖'],
    ]) {
...
async function applySort(page, sort, emitLog) {
  const labels = { newest: '新发布', priceDrop: '新降价' };
...
async function scrapeCurrentPage(page) {
  ...
  return page.evaluate(() => {
    ...
    cards.forEach(card => {
      ...
      const sellerAvatar = card.querySelector('[class*="row4-wrap-seller"] img[class*="avatar"]');
      ...
      items.push({
        ...
        updatedAt: '',
        sellerLastSeen: '',
        // 搜索列表这个位置实际是地区，不是卖家昵称。不能把地区作为卖家身份写入卖家池；
        // 完整昵称和卖家 ID 只由细筛详情页取得。
        sellerName: '',
        sellerLocation: sellerWrap?.getAttribute('title') || sellerWrap?.textContent?.trim() || '',
        sellerAvatarUrl: sellerAvatar?.getAttribute('src') || '',
        sellerFingerprint: canonicalAvatarFingerprint(sellerAvatar?.getAttribute('src') || ''),
        image: imgEl?.src || '',
        badges: ['个人闲置', '验货宝', '验号担保', '包邮', '超赞鱼小铺', '全新', '严选', '转卖'].filter(label => cardText.includes(label)),
        personalSeller: cardText.includes('个人闲置'),
      });
    });
