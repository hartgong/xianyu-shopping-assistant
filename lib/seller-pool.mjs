...
function saveLocal(data) {
  ...
  saveConfig({ sellerProfiles: profiles, sellerManualLabels: labels });
}
...
export async function syncSharedSellerPool() {
  ...
  try { await schema(d); const data = mergeRemote(await remote(d), localSellers()); await saveRemote(d, data); saveLocal(data); initialized = true; lastError = ''; return { enabled: true, profiles: data.sellers.size, fingerprints: data.fps.size }; }
...
export function queueSharedSellerPoolSync() { syncQueue = syncQueue.then(syncSharedSellerPool).catch(e => console.error(`⚠️ 共享卖家池队列失败：${e.message}`)); return syncQueue; }
