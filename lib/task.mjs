...
import { getConfig, getSafeConfig, saveConfig } from './config.mjs';
...
export class TaskManager {
  constructor(broadcast) {
    ...
    this._loadFromDisk();
    this._migrateHistoricalSellerProfiles();
    this._removeInvalidSellerProfiles();
  }
  ...
  _removeInvalidSellerProfiles() {
    ...
    if (changed) {
      saveConfig({ sellerProfiles: profiles });
  ...
  _migrateHistoricalSellerProfiles() {
    ...
    if (changed) {
      saveConfig({ sellerProfiles: existing });
      void queueSharedSellerPoolSync();
  ...
  async startTask(id, configUpdates = {}) {
    ...
    try {
      ...
      if (sellerPool.enabled) emitLog(`☁️ 已同步共享卖家池：${sellerPool.profiles} 条档案`);
