...
function judgeByHardRules(product, options = {}) {
  ...
  const includeWords = normalizeWordList(options.titleInclude);
  if (includeWords.length > 0 && !includeWords.some(word => title.includes(word))) {
    return { pass: false, reason: `标题未包含: ${includeWords.join('/')}` };
  }
  ...
  if (options.wantMax != null && Number.isFinite(Number(product.wantCount)) && Number(product.wantCount) > options.wantMax) {
    return { pass: false, reason: `想要人数高于 ${options.wantMax}` };
  }
